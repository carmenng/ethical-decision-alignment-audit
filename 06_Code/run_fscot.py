...
# Representative audit code for PROMPTING = Few-Shot Chain-of-Thought (FSCOT)

# This notebook demonstrates the audit procedure using one LLM (GPT-4o) as example.
# The procedure shows (1) forced A/B decision parsing, (2) repeated sampling (100 runs), (3) retry/timeout handling, and (4) CSV logging.
...
# max_tokens and retry limits were increased to accommodate few-shot chain-of-thought prompts.
...
# List of expected files for FSCOT scenarios (only YO for this run)
expected_file_names = [
    'YO_FSCOT_EN.csv',
    'YO_FSCOT_CN.csv',
    'YO_FSCOT_JP.csv',
...
# Function to parse response with simplified validation
...
# Function to query GPT-4o with retry logic (retries until valid response with timeout)
def query_gpt4o(prompt, scenario_id, language, run, max_attempts=10, timeout_seconds=60):
    start_time = time.time()
    attempt = 0
    failed_attempts = 0
    while attempt < max_attempts:
        # Check if exceeding timeout
        ...
        try:
            response = client.chat.completions.create(
                ...
                temperature=0.7,
                max_tokens=300  # Increase to 300
            )
            ...
            failed_attempts += 1
            time.sleep(0.5)
    # If max_attempts is reached, raise an exception to indicate failure
...
# Main testing loop: Process only YO dilemma with exact language order, also track failed attempts
dilemma = 'YO'
...
for language in languages:
    file_name = f"{dilemma}_FSCOT_{language}.csv"
    ...
    for index, row in df.iterrows():
        ...
        for run in range(1, 101):
            ...
            temp_df.to_csv(f"{dilemma}_FSCOT_results_gpt4o_temp.csv", index=False)
...
results_df.to_csv(f"{dilemma}_FSCOT_results_gpt4o.csv", index=False)