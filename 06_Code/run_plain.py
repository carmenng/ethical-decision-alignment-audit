# Representative audit execution for PROMPTING = PLAIN
...
# To run: place CSVs in data/inputs/ and provide your own API key(s).
...
DATA_DIR = "data/inputs"  # folder that contains the CSV files

# List of expected files for MF_PLAIN scenarios
# Filenames encode: dilemma_prompt_language.csv
# (e.g., MF_PLAIN_EN.csv)
expected_file_names = [
    'MF_PLAIN_EN.csv',
    'MF_PLAIN_CN.csv',
...
for language in languages:
    file_name = f"{dilemma}_PLAIN_{language}.csv"
    ...
    for index, row in df.iterrows():
        ...
        for run in range(1, 101):
            ...
            # Save intermediate results continuously to avoid data loss
            temp_df = pd.DataFrame(results, columns=['scenario_id', 'language', 'run_number', 'decision', 'explanation'])
            temp_df.to_csv(f"{dilemma}_PLAIN_results_gpt4o_temp.csv", index=False)
...
results_df.to_csv(f"{dilemma}_PLAIN_results_gpt4o.csv", index=False)
